...
class TiDevicePage:
    ...
    def device(self):
        d = TiDevice()
        if len(d.list()) == 0:
            self.edit_output.append("设备为空，TiDevice 初始化失败！")
            self.edit_output.moveCursor(QTextCursor.End)
            self.notice.error("当前设备为空，TiDevice 初始化失败！")
            return
        return d
    ...
    def clicked_btn_install(self):
        ...
        if self.device():
            try:
                ...
                logging.info(f"安装失败, {e}.")
                self.notice.error(f"安装失败, {e}.")
    ...
    def clicked_btn_qrcode(self):
        ...
        if text:
            ...
            self.notice.info("文本框内容为空，不生成二维码！")
