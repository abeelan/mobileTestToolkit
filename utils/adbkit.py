...
class AdbKit:
    ...
    def open_proxy(self, hostname, port):
        """打开手机全局代理"""
        logging.info(f"打开全局代理 {hostname}:{port}")
        self.adb_device.shell(f"settings put global http_proxy {hostname}:{port}")

    def close_proxy(self):
        """关闭手机代理，不需要重启设备生效"""
        self.adb_device.shell(f"settings put global http_proxy :0")
        logging.info(f"关闭全局代理（亲测如不生效重启应用即可）")

    def delete_proxy(self):
        """删除手机代理，删除后需要重启设备"""
        # adb shell settings delete global global_http_proxy_host  # 仅删除 hostname
        # adb shell settings delete global global_http_proxy_port  # 仅删除 port
        self.shell(f"settings delete global http_proxy")
